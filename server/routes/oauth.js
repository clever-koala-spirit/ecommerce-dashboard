...
async function fetchPlatformInfo(platform, tokenData, req = {}) {
  ...
  try {
    if (platform === 'meta') {
      ...
      // Auto-discover GA4 property via Admin API, fallback to Data API
      try {
        // Try Admin API first
        ...
        if (accountsData.accountSummaries?.length > 0) {
          ...
          console.warn('[OAuth] GA4 Admin API error:', accountsData.error.message?.substring(0, 100));
          // Token saved — property ID can be set manually via Settings
          credentials.needsPropertyId = true;
        }
      } catch (error) {
