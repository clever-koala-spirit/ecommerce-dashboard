...
if (typeof window !== 'undefined' && window.location?.hostname === 'localhost') {
  ...
  if (import.meta.env.DEV) {
    if (import.meta.env.DEV) console.log('[MockData] Summary:');
    if (import.meta.env.DEV) console.log(`  Total Revenue: $${Math.round(totalRevenue).toLocaleString()}`);
    if (import.meta.env.DEV) console.log(`  Total Ad Spend: $${Math.round(totalAdSpend).toLocaleString()} (Meta: $${Math.round(totalMetaSpend).toLocaleString()}, Google: $${Math.round(totalGoogleSpend).toLocaleString()})`);
    if (import.meta.env.DEV) console.log(`  Meta % of spend: ${((totalMetaSpend / totalAdSpend) * 100).toFixed(1)}%`);
    if (import.meta.env.DEV) console.log(`  Blended ROAS: ${(totalAdRevenue / totalAdSpend).toFixed(2)}`);
    if (import.meta.env.DEV) console.log(`  Avg AOV: $${(totalRevenue / shopifyData.reduce((s, d) => s + d.orders, 0)).toFixed(2)}`);
    if (import.meta.env.DEV) console.log(`  Last 7 days avg: $${Math.round(last7Revenue / 7).toLocaleString()}/day`);
    if (import.meta.env.DEV) console.log(`  Days of data: ${shopifyData.length}`);
  }
}
