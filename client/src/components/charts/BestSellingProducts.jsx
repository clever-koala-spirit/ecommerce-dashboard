  ...
  if (topProducts.length === 0) {
    return (
      <div className="rounded-2xl p-6" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)' }}>
        <h3 className="font-semibold mb-4" style={{ color: 'var(--color-text-primary)' }}>Best Selling Products</h3>
  ...
  return (
    <div className="rounded-2xl p-6" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', transition: 'background-color 0.3s ease, border-color 0.3s ease' }}>
      <div className="flex items-center justify-between mb-5">
        <h3 className="font-semibold" style={{ color: 'var(--color-text-primary)' }}>Best Selling Products</h3>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="text-left category-label">
            ...
            {topProducts.map((product, idx) => (
              ...
              >
                <td className="py-3 pr-4">
                  <span className="text-xs font-mono" style={{ color: 'var(--color-text-muted)' }}>{product.id}</span>
                </td>
                <td className="py-3 pr-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium truncate max-w-[200px]" style={{ color: 'var(--color-text-primary)' }}>{product.name}</span>
                  </div>
                </td>
                <td className="py-3 pr-4 text-right">
                  <span className="text-sm font-semibold" style={{ color: 'var(--color-text-primary)' }}>{product.sold.toLocaleString()}</span>
                </td>
                <td className="py-3 pr-4 text-right">
                  <span className="text-sm font-semibold" style={{ color: 'var(--color-text-primary)' }}>{formatCurrency(product.revenue)}</span>
                </td>
