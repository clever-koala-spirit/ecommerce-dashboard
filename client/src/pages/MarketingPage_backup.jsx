...
export default function MarketingPage() {
  ...
  const [activeTab, setActiveTab] = useState('attribution'); // Start with attribution engine
    ...
    <div className="p-6 lg:p-8 space-y-6 page-content" style={{ maxWidth: '1440px' }}>
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: colors.textSecondary }}>Advanced attribution modeling and marketing analytics</p>
        </div>
        
        {/* Tab Navigation */}
        <div className="flex rounded-xl p-1" style={{ background: colors.theme === 'light' ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.06)' }}>
          <button
            onClick={() => setActiveTab('attribution')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              activeTab === 'attribution' ? 'shadow-sm' : ''
            }`}
            style={{
              background: activeTab === 'attribution' ? colors.bgCard : 'transparent',
              color: activeTab === 'attribution' ? colors.text : colors.textSecondary,
              border: activeTab === 'attribution' ? `1px solid ${colors.border}` : '1px solid transparent'
            }}
          >
            🧠 Attribution Engine
          </button>
          <button
            ...
            style={{
              background: activeTab === 'overview' ? colors.bgCard : 'transparent',
              color: activeTab === 'overview' ? colors.text : colors.textSecondary,
              border: activeTab === 'overview' ? `1px solid ${colors.border}` : '1px solid transparent'
            }}
          >
            📊 Channel Overview
          </button>
          ...
          {!hasData ? (
            <div className="rounded-2xl p-8" style={{ background: colors.bgCard, border: `1px solid ${colors.border}` }}>
              <EmptyState 
                ...
                message="Link Meta Ads and Google Ads to unlock advanced multi-touch attribution insights and see the complete customer journey." 
              />
            </div>
          ) : (
